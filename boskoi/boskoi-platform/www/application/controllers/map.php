...
class Map_Controller extends Template_Controller {
    ...
    public function index()
    {
				...
				->find_all() as $category)
		{
			...
			foreach ($category->children as $child)
			{
				$children[$child->id] = array(
					...
					$child->category_title_la,
					$child->category_title_nl,
			...
			$parent_categories[$category->id] = array(
				...
				$category->category_title_la,
				$category->category_title_nl,
		...
		$query = $db->query('SELECT incident_date FROM '.$this->table_prefix.'incident WHERE incident_active = 1 GROUP BY DATE_FORMAT(incident_date, \'%Y-%m\') ORDER BY incident_date DESC LIMIT 1');
		//end boskoi
		foreach ($query as $query_active)
		{
			...
			//start boskoi
			$firstQuery = $db->query('SELECT incident_date FROM '.$this->table_prefix.'incident WHERE incident_active = 1 GROUP BY DATE_FORMAT(incident_date, \'%Y-%m\') ORDER BY incident_date ASC LIMIT 1');
			foreach ($firstQuery as $first_active){
				$first_month = date('n', strtotime($first_active->incident_date));
				$first_year = date('Y', strtotime($first_active->incident_date));
				$active_startDate = strtotime($first_year . "-" . $first_month . "-01"); //strtotime($active_year . "-" . $active_month . "-01");
			}
			//end boskoi
		...
		foreach ($query as $slider_date)
		{
			...
			for ( $i=1; $i <= 12; $i++ ) {
				...
				if($first_month && ((int) $i == $first_month) && $first_year == $years){
				//end boskoi
		...
		$this->themes->js = new View('map_js');
