	...
	{	
		...
		if ($_POST)
		{
			...
			if ($post->action == 'a')		// Add Action
			{
				...
				$post->add_rules('category_title_nl','required', 'length[3,80]');
				$post->add_rules('category_title_la','required', 'length[3,80]');
			...
			if ($post->validate())
			{
				...
				elseif( $post->action == 'a' )
				{
					...
					$category->category_title_nl = $post->category_title_nl;
					$category->category_title_la = $post->category_title_la;
